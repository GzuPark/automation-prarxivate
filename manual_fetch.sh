...
if [ ! -d ./prarxivate ]
then 
    echo "Add submodule prarxivate"
    git submodule add https://github.com/GzuPark/prarxivate.git
fi
...
# fetch
python prarxivate/fetch_papers.py -si $si -mi $mi -break 0 -sq cat:cs.CV+OR+cat:cs.RO+OR+cat:cs.AI+OR+cat:cs.LG+OR+cat:cs.CL+OR+cat:cs.NE+OR+cat:stat.ML
